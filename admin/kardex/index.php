...
foreach ($kardexs as $kardex) {
    if($email_sesion == $kardex['email'] || $rol_sesion_usuario == 'ADMINISTRADOR') {
        if($kardex['observacion'] == 'DISCIPLINA') $count_disciplina++;
        if($kardex['observacion'] == 'ASISTENCIA') $count_asistencia++;
        if($kardex['observacion'] == 'RENDIMIENTO ACADÉMICO') $count_rendimiento++;
    }
}
                ...
                <div class="col-md-12">
                    <div class="card card-outline card-success">
                        <div class="card-header">
                            <h3 class="card-title">Estadísticas de Reportes</h3>
                        </div>
                        <div class="card-body">
                            <canvas id="kardexChart" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <table id="example2" class="table table-striped table-bordered table-hover table-sm">
                                ...
                                foreach ($asignaciones as $asignacione){
                                    ...
                                    if($email_sesion == $asignacione['email'] || $rol_sesion_usuario == 'ADMINISTRADOR'){
                                                                        ...
                                                                        <div class="row">
                                                                            <div class="col-md-12">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <input type="text" name="docente_id" value="<?=$kardex['docente_id'];?>" hidden>
...
?>
<!-- ChartJS -->
<script src="<?=APP_URL;?>/public/plugins/chart.js/Chart.min.js"></script>
...
<script>
    $(function () {
        ...
        //- PIE CHART -
        //-------------
        var pieChartCanvas = $('#kardexChart').get(0).getContext('2d');
        var pieData = {
            labels: [
                'DISCIPLINA',
                'ASISTENCIA',
            ...
            datasets: [
                {
                    data: [<?=$count_disciplina;?>, <?=$count_asistencia;?>, <?=$count_rendimiento;?>],
