                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select name="nivel_id" id="nivel_select" class="form-control">
                                                <?php
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="grado_id" id="grado_select" class="form-control">
                                                ...
                                                foreach ($grados as $grado){ ?>
                                                    <option value="<?=$grado['id_grado'];?>" data-nivel-id="<?=$grado['nivel_id'];?>" <?=$grado['id_grado']==$grado_id ? 'selected' : ''?> ><?=$grado['curso']." | Paralelo ".$grado['paralelo'];?></option>
                                                    <?php
                                                }
                                                ?>
                                            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const allGrados = Array.from(gradoSelect.options); // Keep all original options

    function updateGrados() {
        const selectedNivel = nivelSelect.value;
        const currentGrado = gradoSelect.value; // Store currently selected grado
        gradoSelect.innerHTML = ''; // Clear current options
        let firstMatch = null;
        let selectedFound = false;

        allGrados.forEach(option => {
            if (option.getAttribute('data-nivel-id') === selectedNivel || !option.getAttribute('data-nivel-id')) {
                const clonedOption = option.cloneNode(true);
                gradoSelect.appendChild(clonedOption);
                
                // Keep selection if valid
                if (clonedOption.value === currentGrado && clonedOption.value !== "") {
                    clonedOption.selected = true;
                    selectedFound = true;
                }
                
                if (!firstMatch && option.getAttribute('data-nivel-id')) {
                    firstMatch = clonedOption;
                }
            }
        });

        // Auto-select first if currently selected is no longer valid
        if (!selectedFound && firstMatch) {
            firstMatch.selected = true;
        }
    }

    // Trigger on change
    nivelSelect.addEventListener('change', updateGrados);
